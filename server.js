...
async function handleSuccessfulPayment(session) {
    const pendingOrders = readJSON(PENDING_ORDERS_FILE, []);
    const pendingOrder = pendingOrders.find(o => o.stripeSessionId === session.id);
    ...
    if (pendingOrder) {
        ...
        // Remove from pending
        const updatedPending = pendingOrders.filter(o => o.stripeSessionId !== session.id);
        writeJSON(PENDING_ORDERS_FILE, updatedPending);
        
        console.log(`✅ Order ${pendingOrder.id} completed via Stripe`);
    }
}
...
app.post('/api/stripe/create-checkout-session', async (req, res) => {
    try {
        ...
        const lineItems = items.map(item => {
            ...
            return {
                price_data: {
                    ...
                    unit_amount: product.price * 100, // Stripe uses cents/ören
                },
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            mode: 'payment',
            success_url: `${BASE_URL}/success.html?session_id={CHECKOUT_SESSION_ID}&order_id=${orderId}`,
        ...
        writeJSON(PENDING_ORDERS_FILE, pendingOrders);
...
app.get('/api/stripe/session/:sessionId', async (req, res) => {
    try {
        ...
        // If payment successful, ensure order is created
...
app.get('/api/orders/:id', (req, res) => {
    ...
    const order = orders.find(o => o.id === req.params.id) || 
                  pendingOrders.find(o => o.id === req.params.id);
...
app.post('/api/change-password', requireAuth, (req, res) => {
    ...
    const admins = readJSON(ADMIN_FILE, []);
    const adminIndex = admins.findIndex(a => a.username === session.username);
...
app.listen(PORT, () => {
    console.log(`🍵 Tegrevinnan server running on ${BASE_URL}`);
    if (STRIPE_SECRET_KEY === 'sk_test_PLACEHOLDER') {
        console.log('⚠️  Stripe is not configured. Set STRIPE_SECRET_KEY environment variable.');
    } else {
